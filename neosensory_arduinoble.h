...
typedef struct ble_gap_evt_adv_report {
...
class NeosensoryBluefruit
{
    ...
    typedef void (*ReadNotifyCallback)(const uint8_t*, uint16_t);
  ...
  public:
    ...
    */
     ...
     */
    void readNotifyCallback(BLEDevice central, BLECharacteristic characteristic);
  ...
  private:
    ...
    bool checkDevice(BLEDevice peripheral);
    bool checkIsNeosensory(BLEDevice peripheral);
    ...
    void parseCliData(const uint8_t* data, uint16_t len);
    ...
    uint8_t wb_service_uuid_[17];
    uint8_t wb_write_char_uuid_[17];
    uint8_t wb_read_char_uuid_[17];

    rtos::Thread poll_thread_;

    /* Services & Characteristic */
    // BLEClientService wb_service_;
    BLECharacteristic wb_write_characteristic_;
    BLECharacteristic wb_read_characteristic_;
