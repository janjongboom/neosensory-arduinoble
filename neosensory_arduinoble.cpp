...
//  , wb_read_characteristic_(wb_read_char_uuid_)
{
	...
	poll_thread_.start(&blePollWrapper);
}
...
void NeosensoryBluefruit::begin(void) {
	...
	BLE.setEventHandler(BLEDiscovered, scanCallbackWrapper);
...
bool NeosensoryBluefruit::isConnected(void) {
	return peripheral_ != NULL && peripheral_->connected();
}
...
bool NeosensoryBluefruit::checkAddressMatches(uint8_t* foundAddress) {
	for (int i = 0; i < BLE_GAP_ADDR_LEN; i++) {
		if (device_address_[i] != foundAddress[BLE_GAP_ADDR_LEN - (i + 1)]) {
			return false;
		}
	}
	return true;
 ...
 *  advertising data.
 */
bool NeosensoryBluefruit::checkIsNeosensory(BLEDevice peripheral) {
	if (!peripheral.hasLocalName()) {
		// Serial.print("Found device with no name: ");
		// Serial.println(peripheral.address());
		return false;
	}

	String name = peripheral.localName();
	char local_name[33] = { 0 };
	peripheral.localName().toCharArray(local_name, 32, 0);

	if (strstr(local_name, "Buzz")) {
 ...
 *  @param[in] report The found report
 */
bool NeosensoryBluefruit::checkDevice(BLEDevice peripheral) {
	uint8_t mac[6] = { 0 };
	char mac_addr[33] = { 0 };
	peripheral.address().toCharArray(mac_addr, 32, 0);
	sscanf((const char*)mac_addr, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5]);

	if (connect_to_any_neo_device_) {
		return checkIsNeosensory(peripheral);
	} else {
		return checkAddressMatches(mac);
	}
}
...
void NeosensoryBluefruit::sendCommand(char cmd[]) {
	wb_write_characteristic_.writeValue(cmd, strlen(cmd));
}

void NeosensoryBluefruit::authorizeDeveloper(void) {
    Serial.println("authorizeDeveloper");
	sendCommand("auth as developer\n");
    Serial.println("authorizeDeveloper sent");
}
 ...
 */
void NeosensoryBluefruit::parseCliData(const uint8_t* data, uint16_t len) {
	for (int i = 0; i < len; i++) {
		if (data[i] == '{') {
			jsonStarted_ = true;
			jsonMessage_ = "";
...
void NeosensoryBluefruit::scanCallback(BLEDevice peripheral)
{
	if (checkDevice(peripheral)) {
		BLE.stopScan();

		Serial.println("Found buzz, trying to connect...");
		...
		wb_read_characteristic_ = peripheral.characteristic("6e400003-b5a3-f393-e0a9-e50e24dcca9e");
		wb_write_characteristic_ = peripheral.characteristic("6e400002-b5a3-f393-e0a9-e50e24dcca9e");
		if (!wb_read_characteristic_) {
			Serial.println("Characteristic not found");
			externalConnectedCallback(false);
            peripheral.disconnect();
            BLE.scan(true);
			return;
		}
		if (!wb_read_characteristic_.canSubscribe()) {
			Serial.println("Cannot subscribe");
			externalConnectedCallback(false);
            BLE.scan(true);
			return;
		}
		if (!wb_read_characteristic_.subscribe()) {
			Serial.println("Failed to subscribe to read characteristic");
			externalConnectedCallback(false);
            BLE.scan(true);
			return;
		}

		wb_read_characteristic_.setEventHandler(BLEWritten, &readNotifyCallbackWrapper);
		...
		if (externalConnectedCallback) {
			externalConnectedCallback(true);
		}
	}
...
void NeosensoryBluefruit::readNotifyCallback(BLEDevice central, BLECharacteristic characteristic) {
...
void scanCallbackWrapper(BLEDevice peripheral) {
	NeosensoryBluefruit::NeoBluefruit->scanCallback(peripheral);
}

void readNotifyCallbackWrapper(BLEDevice central, BLECharacteristic characteristic) {
	NeosensoryBluefruit::NeoBluefruit->readNotifyCallback(central, characteristic);
}
